    ...
    T: Send + Sync,
{
    ...
    pub fn poll_once(&mut self) -> TaskPoll<T> {
        if let Some(task) = self.task.as_mut() {
            block_on(poll_once(task))
                .map(TaskPoll::Done)
                .unwrap_or(TaskPoll::Pending)
        } else {
            TaskPoll::Canceled
        }
    }

    pub async fn cancel_and_get(&mut self) -> Option<T> {
        let cancellation = self.task.take().map(|task| task.cancel());
...
impl<T> TaskPoll<T> {
    pub fn is_done(&self) -> bool {
        matches!(self, TaskPoll::Done(_))
    }
    pub fn is_pending(&self) -> bool {
        matches!(self, TaskPoll::Pending)
    }
    pub fn is_canceled(&self) -> bool {
        matches!(self, TaskPoll::Canceled)
    }
    pub fn unwrap(self) -> T {
        match self {
            TaskPoll::Done(value) => value,
            _ => panic!("called `TaskPoll::unwrap()` on a `Pending` or `Canceled` value"),
        }
    }
    ...
    pub fn as_ref(&self) -> Option<&T> {
        match self {
            TaskPoll::Done(value) => Some(value),
            _ => None,
        }
    }
}

impl<T> From<TaskPoll<T>> for Option<T> {
    fn from(value: TaskPoll<T>) -> Self {
        match value {
            TaskPoll::Pending => None,
            TaskPoll::Done(value) => Some(value),
            TaskPoll::Canceled => None,
        }
    }
}
